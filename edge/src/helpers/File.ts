...
export function readFile (r: NginxHTTPRequest, path: string, options: object = {}): string|void {
  try {
    return fs.readFileSync(path, options) as unknown as string
  } catch (e) {
    r.error(`Could not read file <${path}>: ${e.message}`)
    throw e
  }
}
